...
Keep it to 1-2 authoritative sentences.`;
  ...
  const filteredBots = useMemo(() => {
    ...
    return bots.filter(bot => {
      // Short-circuit: Check status first (O(1)) before string matching (O(N))
      const matchesStatus = isAllStatus || bot.status === statusFilter;
      if (!matchesStatus) return false;

      // Only perform string matching if status matches
