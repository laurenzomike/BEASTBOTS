...
Keep it to 1-2 authoritative sentences.`;
  ...
  // Optimization: Memoize filteredBots to prevent O(N) filtering on every render.
  // Extract searchQuery.toLowerCase() out of the loop and short-circuit the status
  // check before running expensive string operations to improve performance.
  const filteredBots = useMemo(() => {
    ...
    return bots.filter(bot => {
      ...
      return bot.name.toLowerCase().includes(query) || bot.type.toLowerCase().includes(query);
    });
