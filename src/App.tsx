...
Keep it to 1-2 authoritative sentences.`;
  ...
  // Extracts invariant `toLowerCase()` string manipulation out of the filter loop
  // and wraps the array filtering in useMemo to prevent O(N) calculations on every React render
  // unless the underlying bots list, search query, or filter changes.
  const filteredBots = useMemo(() => {
    const lowerQuery = searchQuery.toLowerCase();
    const isAll = statusFilter === "all";

    return bots.filter(bot => {
      if (!isAll && bot.status !== statusFilter) return false;
