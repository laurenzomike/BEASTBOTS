...
Keep it to 1-2 authoritative sentences.`;
  ...
  const filteredBots = useMemo(() => {
    ...
    const checkStatus = statusFilter !== "all";
    const hasQuery = lowerQuery.length > 0;

    return bots.filter(bot => {
      // Fast path: status mismatch
      if (checkStatus && bot.status !== statusFilter) return false;
      // Fast path: no search query
      if (!hasQuery) return true;

      // Expensive string matching only if necessary
