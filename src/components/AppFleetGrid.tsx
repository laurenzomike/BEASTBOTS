...
export const AppFleetGrid: React.FC<AppFleetGridProps> = ({
  ...
  // ⚡ Bolt: Consolidate active and standby bot filtering into a single O(N) pass
  ...
  // ⚡ Bolt: Pre-calculate latest activity per bot to eliminate O(N*M) lookup in render loop
  const latestActivityMap = React.useMemo(() => {
    const map = new Map<string, string>();
    // Iterate forwards to match Array.prototype.find() behavior (first match wins)
  ...
  // ⚡ Bolt: Consolidate fleet summary calculations into a single O(N) pass
  const fleetSummary = React.useMemo(() => {
    ...
    for (let i = 0; i < bots.length; i++) {
      ...
      if (b.config?.winCount) winCount += b.config.winCount;
      if (b.config?.workflows?.length) workflowCount += b.config.workflows.length;
    }
    return { onlineCount, winCount, workflowCount };
  }, [bots]);
    ...
    >
      ...
      <div className="hidden lg:grid grid-cols-2 xl:grid-cols-4 gap-6 px-12 pt-12">
         {[
           ...
           { label: "Active Cycles", value: fleetSummary.onlineCount, color: "bg-[var(--brand)]", detail: "Online Units" },
           { label: "Protocol Wins", value: fleetSummary.winCount, color: "bg-[#00E0FF]", detail: "Strategic Success" },
           { label: "Active Threads", value: fleetSummary.workflowCount, color: "bg-[#FF2E00] text-white", detail: "Running Automations" }
         ].map(stat => (
