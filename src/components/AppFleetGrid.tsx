...
export const AppFleetGrid: React.FC<AppFleetGridProps> = ({
  ...
  const globalActivitiesMap = React.useMemo(() => {
  ...
  const fleetStats = React.useMemo(() => {
    ...
    for (const bot of bots) {
      if (bot.status === 'online') activeCycles++;
      protocolWins += (bot.config?.winCount || 0);
      activeThreads += (bot.config?.workflows?.length || 0);
    }
        ...
        <div className="px-4 lg:px-12 mt-12">
          ...
          >
            {activeBots.map((bot, i) => {
              const lastLog = globalActivitiesMap.get(bot.id) || "";
              return (
                <motion.div 
                  key={bot.id}
                  variants={{
