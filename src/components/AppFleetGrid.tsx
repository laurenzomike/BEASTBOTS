...
export const AppFleetGrid: React.FC<AppFleetGridProps> = ({
  ...
  // ⚡ Bolt: Memoize filtering to prevent O(N) recalculations on every render
  const activeBots = React.useMemo(() =>
    filteredBots.filter(b => b.status === 'online' || b.status === 'error'),
  [filteredBots]);

  const standbyBots = React.useMemo(() =>
    filteredBots.filter(b => b.status === 'offline' || b.status === 'auth-required'),
  [filteredBots]);

  // ⚡ Bolt: Replace O(N*M) nested loop with O(N+M) Map lookup
  const latestActivitiesByBotId = React.useMemo(() => {
        ...
        <div className="px-4 lg:px-12 mt-12">
          ...
          >
            {activeBots.map((bot, i) => {
              const lastLog = latestActivitiesByBotId.get(bot.id) || "";
              return (
                <motion.div 
                  key={bot.id}
                  variants={{
