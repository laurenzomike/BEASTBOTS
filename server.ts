...
// Helper to prevent Reflected XSS
...
app.get(["/api/oauth/:provider/callback", "/api/oauth/:provider/callback/"], async (req, res) => {
  ...
  try {
    ...
    if (tokens.error) {
      ...
      const safeErrorDesc = escapeHtml(tokens.error_description || tokens.error);
      return res.status(400).send(`<html><body><h3>Authentication Error</h3><p>${safeErrorDesc}</p></body></html>`);
    }
