...
app.get(["/api/oauth/:provider/callback", "/api/oauth/:provider/callback/"], async (req, res) => {
  ...
  try {
    ...
    if (tokens.error) {
      console.error(`Oauth Token Error [${provider}]:`, tokens.error_description || tokens.error || "Unknown token error");
      const errorMessage = String(tokens.error_description || tokens.error || "Unknown error")
        .replace(/&/g, "&amp;")
        .replace(/</g, "&lt;")
        .replace(/>/g, "&gt;")
        .replace(/"/g, "&quot;")
        .replace(/'/g, "&#39;");
      return res.status(400).send(`<html><body><h3>Authentication Error</h3><p>${errorMessage}</p></body></html>`);
    }
